...
Asena.addCommand({pattern: 'update$', fromMe: true, deleteCommand: false, desc: Lang.UPDATER_DESC}, (async (message, match) => {
    await git.fetch();
...
Asena.addCommand({pattern: 'update now$', fromMe: true, deleteCommand: false, desc: Lang.UPDATE_NOW_DESC, dontAddCommandList: true}, (async (message, match) => {
    await git.fetch();
    var commits = await git.log([Config.BRANCH + '..origin/' + Config.BRANCH]);
    if (commits.total === 0) {
